...
struct ContentView: View {
    ...
    var body: some View {
        ZStack {
            if isFetched == false {
                // Показываем загрузку пока идет проверка сервера
                ZStack {
                    Color(hex: "#3e4464")
                        .ignoresSafeArea()
                ...
                if isBlock == true {
                    // Показываем наше обычное приложение
        ...
        session.dataTask(with: request) { data, response, error in
            ...
            DispatchQueue.main.async {
                ...
                if let error = error {
                    ...
                    print("Server unavailable, showing app")
                    self.isBlock = true
                    self.isFetched = true
                    return
                }
                ...
                if let httpResponse = response as? HTTPURLResponse {
                    ...
                    if httpResponse.statusCode == 200 {
                        ...
                        if contentLength == "0" || !hasContent {
                            ...
                            print("🚫 Empty response (do nothing): Showing app")
                            self.isBlock = true
                            self.isFetched = true
                        } else {
                            // Есть контент = успех
                        ...
                        print("🚫 Error code \(httpResponse.statusCode)): Showing app")
                        self.isBlock = true
                        self.isFetched = true
                    }
                    ...
                    print("❌ No HTTP response: Showing app")
                    self.isBlock = true
                    self.isFetched = true
                }
            }
